...
def train_by_single_sampel():
	...
	for i in range(n_sample):
		...
		if i%1000 == 0:
			...
			h_prob_0 = sigmoid(np.dot(x, W) + hb) 
			h_samp_0 = gibbs_sample(h_prob_0)
			test_h_prob = sigmoid(np.dot(test_x, W) + hb)
			test_h_samp = gibbs_sample(test_h_prob)
			softmax_LR_sklearn(h_samp_0, train_y, test_h_samp, test_y)
			softmax_LR_sklearn(h_prob_0, train_y, test_h_prob, test_y)
